...
object App {
    init {
        ...
        val testLocation1 = Location("test1", 20.3, 13.5, mutableListOf())
        val testLocation2 = Location("test2", -10.0, 0.0, mutableListOf())
            .apply { address = Address("region2", "country2", "area2", null, this) }
        val testLocation3 = Location("test3", 0.0, 1000.0, mutableListOf())
            .apply { address = Address("someRegion3", "someCountry3", "someArea3", "someCity3", this) }

        val testDate1 = ForecastDate(LocalDateTime.now(), LocalDateTime.now(), mutableListOf(), testLocation1)
        ...
        val testDate2 = ForecastDate(LocalDateTime.now().minusDays(1), LocalDateTime.now().plusDays(3), mutableListOf(), testLocation1)
        val testForecast2 = Forecast(-20.0, -17.0, -30.0, 990.0, 40.2, 85.0, WindDir.S, 60, 75, Precipitation.SNOW, 50.0, Provider.WEATHERSTACK, testDate2)
        testDate2.forecast.add(testForecast2)
